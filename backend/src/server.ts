...
function responder(cliente: WebSocket, id: string | undefined, evento: string, datos: unknown): void {
      ...
      switch (accion) {
        ...
        case 'guardar-base-datos':
          responder(
            ...
            id,
            'guardado-base-datos',
            await accionGuardarBaseDatos(datos as any, (registros) => broadcast('nuevos-registros', registros)),
          );
          break;
...
app.use('/api', crearRouterConfig((registros) => broadcast('nuevos-registros', registros)));
