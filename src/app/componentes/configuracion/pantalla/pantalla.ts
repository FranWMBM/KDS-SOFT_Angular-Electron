...
import { BackendService, MonitorCocina } from '../../../servicios/backend-service';
...
export class Pantalla implements OnInit {
  ...
  async cargarMonitores(): Promise<void> {
    ...
    try {
      ...
      if (seleccionado && !this.monitores.some((monitor) => monitor.id === seleccionado)) {
        this.formulario.patchValue({ monitorCocina: null });
      }
    } catch (error) {
      console.error('No se pudieron cargar los monitores:', error);
      this.errorMonitores.set('No se pudieron cargar los monitores. Revisa la conexión.');
    } finally {
  ...
  async guardar(): Promise<void> {
    ...
    try {
      ...
      if (resultado.correcto) {
        ...
        this.mensaje.set(resultado.error ?? 'No se pudo guardar la pantalla.');
      }
    } catch (error) {
